...
function App() {
  ...
  const updateTodo = (id, todo) => {
    ...
    ); // Herre we use duscorde operation like if-else ----- if prevTodo.id === id then return todo else prevTodo
  };
  ...
  const toggleComplete = (id) => {
    ...
    ); // here we check if prevTodo.id is equals id if yes -- we take all prevTodo values and only override the completed value
  };
  ...
  useEffect(() => {
    // we can add more than one useEffect in react
